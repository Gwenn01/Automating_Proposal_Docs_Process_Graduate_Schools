...
import { X, Search, Check, User, Users, Info, ArrowRight } from 'lucide-react';

const AssignModal = ({ isOpen, onClose, data }) => {
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      {/* Standard Modal Container - Single Column */}
      <div className="relative bg-white w-full max-w-[480px] rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="p-8">
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-6">
            ...
            <p className="text-[13px] font-bold text-slate-600 line-clamp-1 italic">
              "{data?.title}"
            </p>
          </div>

          {/* Search Bar - Matching Desktop - 28.png Style */}
          ...
          {/* Selection List - Single Column & Max Height Control */}
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                    selectedReviewer === reviewer.id ? 'bg-green-500 text-white' : 'bg-slate-200 text-slate-500'
                  }`}>
                    <User size={16} />
                  </div>
                  <div className="text-left">
                    <p className={`text-sm font-black ${selectedReviewer === reviewer.id ? 'text-green-700' : 'text-slate-700'}`}>
                      {reviewer.name}
                    </p>
