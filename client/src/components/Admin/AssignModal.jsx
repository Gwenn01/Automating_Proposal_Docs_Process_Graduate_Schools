...
import { X, Search, Check, User, Info, RefreshCcw, UserPlus, Loader2 } from 'lucide-react';
...
const AssignModal = ({ isOpen, onClose, data, onUpdate }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      const fetchReviewers = async () => {
        try {
          const response = await axios.get("http://127.0.0.1:5000/api/get-all-reviewer");
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      console.log("DEBUG: Final Payload being sent to Flask:", JSON.stringify(payload, null, 2));

      const response = await axios.post("http://127.0.0.1:5000/api/assign-reviewer", payload);
      ...
      const backendMessage = error.response?.data?.message || "Invalid Data Error";
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-[480px] rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className={`h-1.5 w-full transition-colors duration-500 ${isReassign ? 'bg-blue-500' : 'bg-green-500'}`} />
        
        <div className="p-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2">
                {isReassign ? <RefreshCcw size={20} className="text-blue-500" /> : <UserPlus size={20} className="text-green-500" />}
                {isReassign ? "Reassign Reviewer" : "Assign Reviewer"}
              </h2>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                {isReassign ? "Update the existing assignment" : "Select an expert for this proposal"}
              </p>
            </div>
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-6 relative overflow-hidden">
            <Info size={40} className={`absolute -right-2 -bottom-2 opacity-5 ${isReassign ? 'text-blue-600' : 'text-green-600'}`} />
            <div className="flex items-center gap-2 mb-1 relative z-10">
              <Info size={12} className={isReassign ? 'text-blue-600' : 'text-green-600'} />
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Target Document</span>
            </div>
            <p className="text-[13px] font-bold text-slate-600 line-clamp-1 italic relative z-10">"{data?.title}"</p>
          </div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3.5 rounded-2xl transition-all duration-200 border-2 ${
                    isSelected 
                      ? isReassign ? 'border-blue-500 bg-blue-50/50' : 'border-green-500 bg-green-50/50' 
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${
                      isSelected 
                        ? isReassign ? 'bg-blue-500 text-white' : 'bg-green-500 text-white' 
                        : 'bg-slate-200 text-slate-500'
                    }`}>
                      <User size={16} />
                    </div>
                    <div className="text-left">
                      <p className={`text-sm font-black ${isSelected ? (isReassign ? 'text-blue-700' : 'text-green-700') : 'text-slate-700'}`}>
                        {reviewer.name}
                      </p>
                      <p className="text-[9px] font-bold text-slate-400 uppercase">{reviewer.dept}</p>
                    </div>
                  </div>
                  {isSelected && <Check size={16} className={isReassign ? 'text-blue-600' : 'text-green-600'} strokeWidth={4} />}
                </button>
              );
            })}
          </div>
