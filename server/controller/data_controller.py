...
def get_data_controller():
    try:
        ...
        # format the data
            ...
            proposal_cover = fetch_proposal_cover_page(proposal_id)[0]
            proposal_content = fetch_proposal_content(proposal_id)[0]
            proposal_review = get_reviews(proposal_id)
            result = get_review_per_docs_mapper(proposal_cover, proposal_content, proposal_review)
            result['status'] = status
            return jsonify(result), 200
